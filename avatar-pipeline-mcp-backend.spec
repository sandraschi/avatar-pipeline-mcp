...
# PyInstaller spec — produce avatar-pipeline-mcp-backend.exe for Tauri NSIS embedding.
# Fleet standard: strip=False, upx=False, noarchive=True (see tauri_nsis_building.md).
# Usage (from repo root):
#   uv run pyinstaller avatar-pipeline-mcp-backend.spec --distpath dist --clean --noconfirm

block_cipher = None

a = Analysis(
    ["src/avatar_pipeline_mcp/__main__.py"],
    pathex=["src"],
    binaries=[],
    datas=[("src/avatar_pipeline_mcp", "avatar_pipeline_mcp")],
    hiddenimports=[
        "uvicorn.logging",
        "uvicorn.loops",
...
exe = EXE(
    ...
    a.datas,
    [],
    name="avatar-pipeline-mcp-backend",
    debug=False,
